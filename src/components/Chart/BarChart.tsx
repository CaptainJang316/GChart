...
const BarChart: React.FC<SingleBarChartProps> = ({
  ...
  axis,
  chartStyle: { color = "green", hoverColor = darkenColor(color, 0.2) },
  tooltip = true,
}) => {
      ...
      >
        ...
        <g className="chart-layer">
          {values.map((d, i) => {
            ...
            if (isHovered)
              console.log("hoveredInfo?.index: " + hoveredInfo?.index);
            ...
            return (
              ...
              <g
                key={i}
                transform={`translate(${i * barWidth}, ${height - barHeight})`}
              >
                <StyledRect
                  ...
                  onMouseOver={() =>
                    setHoveredInfo((prev) => ({ ...prev, index: i }))
                  }
        ...
        <g className="tooltip-layer">
          {tooltip && hoveredInfo?.index != null && (
            <TooltipGroup
              transform={`translate(${hoveredInfo.x}, ${hoveredInfo.y})`}
            >
              <TooltipRect x={-30} y={-25} width={rectWidth} />
              <g ref={titleRef}>
                <circle
                  cx={-15}
                  cy={-10}
                  ...
                  stroke-width="7"
                />
                <TooltipTitle y={-5}>{hoveredInfo.label}</TooltipTitle>
              </g>
              <TooltipText ref={textRef} x={-20} y={15}>
