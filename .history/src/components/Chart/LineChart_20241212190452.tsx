...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const createSolidLinePath = (currLineValues: number[], i: number) => {
    ...
    firstX = currX;
    pathData[i] = `M ${currX} ${scales.yScale(currLineValues[0])}`;

    currLineValues.forEach((d, i) => {
      if (i === 0) return;

      currX += barWidth;
      pathData[i] += ` L ${currX} ${scales.yScale(d)}`;
    });
    lastX = currX;
  ...
  const createSmoothLinePath = (currLineValues: number[], i: number) => {
    ...
    // M 명령으로 시작점 설정
    pathData[i] = `M ${currXValues[0]} ${scales.yScale(currLineValues[0])}`;
    ...
    for (let i = 0; i < currLineValues.length - 1; i++) {
      ...
      // 곡선 연결
      pathData[i] += ` C ${cp1X} ${cp1Y}, ${cp2X} ${cp2Y}, ${x2} ${y2}`;
    }
