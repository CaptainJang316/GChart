...
const LineChart: React.FC<LineChartProps> = ({
    ...
    data.forEach((d, i) => {
        ...
        if (i === 0) {
            pathData = `M ${x} ${scales.yScale(d.value)}`;
        } else {
            // 이전 점과 현재 점 사이의 중간점을 제어점으로 사용
            const x_mid = x - (barWidth / 2);
            pathData += ` C ${x_mid} ${scales.yScale(data[i-1].value)}, ${x_mid} ${scales.yScale(d.value)}, ${x} ${scales.yScale(d.value)}`;
        }
    });
    ...
    for (let i = data.length - 1; i >= 0; i--) {
        ...
        if (i === data.length - 1) {
            pathData += ` L ${x} ${scales.yScale(0)}`;
        } else {
            const x_mid = x + (barWidth / 2);
            pathData += ` C ${x_mid} ${scales.yScale(0)}, ${x_mid} ${scales.yScale(0)}, ${x} ${scales.yScale(0)}`;
        }
    }
