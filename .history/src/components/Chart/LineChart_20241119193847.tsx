...
const StyledPath = styled.path<{
    isVisible?: boolean, 
    totalLength?: number, 
    dataLength?: number,  // 실제 데이터 라인 길이
    color?: string, 
    fillArea?: boolean
}>`
    fill: ${props => props.fillArea ? `${props.color}22` : 'none'};
    stroke: ${props => props.color};    
    stroke-width: 2;
    stroke-dasharray: ${props => props.fillArea ? 
        `${props.dataLength} ${props.totalLength! - props.dataLength!}` : 
        props.totalLength
    };    
    ...
    animation: ${props => 
        ...
        css`
            ...
            ${fillAnimation} 1.5s ease-in-out forwards
        `};
...
const LineChart: React.FC<LineChartProps> = ({
    ...
    const [fillPathLength, setFillPathLength] = useState(0);
    ...
    useEffect(() => {
        if (pathRef.current) {
            // 전체 경로 길이 계산
            const fullLength = pathRef.current.getTotalLength();
            setFillPathLength(fullLength);
            
            // 임시 SVG 엘리먼트를 생성하여 데이터 라인 길이만 계산
            const tempPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
            tempPath.setAttribute("d", createLinePathString());
            document.body.appendChild(tempPath);
            const dataLength = tempPath.getTotalLength();
            document.body.removeChild(tempPath);
            setLinePathLength(dataLength);
        }
    }, [data]);
    ...
    const createPathData = () => {
        ...
        // 영역 채우기를 위한 닫힌 경로 생성 (실제로는 안보이게 처리됨)
        const lastX = axis?.xAxis?.boundaryGap 
            ? 25 + ((data.length - 1) * barWidth)
            : barWidth - (barWidth / 6) - 5 + ((data.length - 1) * barWidth);
        pathData += ` L ${lastX} ${height}`; // 마지막 점에서 아래로
        pathData += ` L ${firstX} ${height}`; // 왼쪽 아래 모서리로
        pathData += ` Z`; // 시작점으로 돌아가기
        
        return pathData;
    };

    // 실제 데이터 라인만의 경로를 생성하는 함수
    const createLinePathString = () => {
        const firstX = axis?.xAxis?.boundaryGap 
            ? 25 
            : barWidth - (barWidth / 6) - 5;
        
        let pathData = `M ${firstX} ${scales.yScale(data[0].value)}`;
    ...
    // 전체 경로 생성 (채우기를 위한 닫힌 경로)
    const createFillPathString = () => {
        const firstX = axis?.xAxis?.boundaryGap 
            ? 25 
            : barWidth - (barWidth / 6) - 5;
            
        let pathData = createLinePathString();
        ...
        <div style={{ display: 'inline-block', width: `${width}px`, height: `${height}px`}}>
            <svg width='100%' height='100%' viewBox={`0 0 ${width} ${height + 30}`} style={{ border: '1px solid #ccc', padding: `${padding}px`}}>
                ...
                <g className='chart-layer'>
                    <StyledPath
                        ref={pathRef}
                        d={createFillPathString()}
                        isVisible={isVisible}
                        lineLength={linePathLength}
                        totalLength={fillPathLength}
                        fillArea={fillArea}
