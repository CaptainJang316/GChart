...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const createSmoothLinePath = (currLineValues: number[]) => {
    ...
    for (let i = 0; i < currLineValues.length - 1; i++) {
        const x0 = i;
        const y0 = currLineValues[i];
        const x1 = i + 1;
        const y1 = currLineValues[i + 1];

        // Calculate tangents (gradients) for Hermite Interpolation
        const m0x = 0;
        const m0y = i === 0 ? 0 : (currLineValues[i + 1] - currLineValues[i - 1]) / 2;
        const m1x = 0;
        const m1y = i === currLineValues.length - 2 ? 0 : (currLineValues[i + 2] - currLineValues[i]) / 2;
        ...
        for (let t = 0; t <= 1; t += 0.1) {
            ...
            const x = h1 * x0 + h2 * x1 + h3 * m0x + h4 * m1x;
            const y = h1 * y0 + h2 * y1 + h3 * m0y + h4 * m1y;
        ...
        if (i === 0) {
            path.push(`M ${segmentPath[0][0]},${segmentPath[0][1]}`);
        }
        for (let j = 1; j < segmentPath.length; j++) {
            path.push(`L ${segmentPath[j][0]},${segmentPath[j][1]}`);
        }
    }
