...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const const createCurvedLinePath = (currLineValues: number[]) => {
    // 시작 X 좌표 설정
    let currX = axis?.xAxis?.boundaryGap ? barWidth / 2 + 25 : 25;
    ...
    currLineValues.forEach((value, index) => {
      ...
      // 부드러운 곡선 경로 생성 (S 명령어 사용)
      pathData += ` S ${currX} ${scales.yScale(value)}, ${currX} ${scales.yScale(value)}`;
    });
  
    return pathData;
  }; = (currLineValues: number[]) => {
    // 시작 X 좌표 설정
    let currX = axis?.xAxis?.boundaryGap ? barWidth / 2 + 25 : 25;
