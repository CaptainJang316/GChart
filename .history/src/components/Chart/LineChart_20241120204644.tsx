...
const LineChart: React.FC<LineChartProps> = ({
    ...
    const labels = data.xLabel.map(item => item.xLabel);
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const barWidth = axis?.xAxis?.boundaryGap? (width - 30) / (values.length-1) : (width - 30) / values.length;
        ...
        const createLinePath = () => {
            ...
            pathData = `M ${firstX} ${scales.yScale(values[0])}`;
        ...
        const createAreaPath = () => {
            ...
            const lastX = axis?.xAxis?.boundaryGap 
                ? 25 + ((values.length - 1) * barWidth)
                : barWidth - (barWidth / 6) - 5 + ((values.length - 1) * barWidth);
        ...
        const calculateAnimationDelay = (index:number) => {
            return (index / (values.length - 1)) * TOTAL_LINE_ANIMATION;
        }
