...
const BarChart: React.FC<Option> = ({
    ...
    const {yAxis, scales} = useAxis({data: values, width: width, height: height, minValue: axis.yAxis?.min, maxValue: axis.yAxis?.max});
    const {gridLines} = useGrid({data: values, scales, width, height, });
