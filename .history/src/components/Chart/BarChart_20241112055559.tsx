...
const BarChart: React.FC<Option> = ({
    ...
    const [hoverInfo, setHoverInfo] = useState<{index: number | null, x: number, y: number}>({
        index: null,
        x: 0,
        y: 0
    });
    ...
    return (
        ...
        <div style={{ display: 'inline-block', width: `${width}px`, height: `${height}px`}}>
            <svg width='100%' height='100%' viewBox={`0 0 ${width} ${height + 30}`} onMouseMove={(e: React.MouseEvent<SVGElement>) => handleMouseMove(e)} style={{ border: '1px solid #ccc', padding: `${padding}px`}}>
                {               
                    title? 
                        ...
                        return (
                            ...
                            <g key={i} transform={`translate(${i * barWidth}, ${height - barHeight})`}>
                                <StyledRect
                                    ...
                                    onMouseEnter={() => setHoverInfo(prev => ({...prev, index: i}))}
                                    onMouseLeave={() => setHoverInfo(prev => ({...prev, index: null}))}
                                    className='bar-rect'
                                />
                                {isHovered && (
