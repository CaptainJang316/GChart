...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const createCurvedLinePath = (currLineValues: number[], options?: { 
    tension?: number,  // 곡선의 급격함 조절 (0-1 사이)
    startOffset?: number 
  }) => {
    const { 
      tension = 0.5,  // 기본 장력 값
      startOffset = barWidth / 2 + 25 
    } = options || {};
    ...
    // 초기 시작점 설정
    let currX = axis?.xAxis?.boundaryGap ? startOffset : 25;
    pathData = `M ${currX} ${scales.yScale(currLineValues[0])}`;
    ...
    currLineValues.forEach((value, index) => {
      if (index === 0) return;
      currX += barWidth;
      controlPoints.push({ x: currX, y: scales.yScale(value) });
    });
  
    // Cardinal Curve 제어점 계산
    const getControlPoint = (
      ...
      const prevToNext = Math.sqrt(
        Math.pow(next.x - prev.x, 2) + Math.pow(next.y - prev.y, 2)
      );
  
      const scale = prevToNext * tension;
      
      return {
        x: curr.x,
        y: curr.y,
        controlIn: {
          x: curr.x - scale * (next.x - prev.x) / prevToNext,
          y: curr.y - scale * (next.y - prev.y) / prevToNext
        },
        controlOut: {
          x: curr.x + scale * (next.x - prev.x) / prevToNext,
          y: curr.y + scale * (next.y - prev.y) / prevToNext
    ...
    controlPoints.forEach((point, index) => {
      const prev = index > 0 ? controlPoints[index - 1] : null;
      const next = index < controlPoints.length - 1 ? controlPoints[index + 1] : null;
  
      const processedPoint = getControlPoint(prev, point, next);
  
      if (index === 0) {
        // 첫 번째 곡선 세그먼트
        pathData += ` C ${processedPoint.controlOut.x} ${processedPoint.controlOut.y}, 
                        ${point.x} ${point.y}, 
                        ${point.x} ${point.y}`;
      } else {
        // 후속 곡선 세그먼트
        pathData += ` S ${processedPoint.controlOut.x} ${processedPoint.controlOut.y}, 
                        ${point.x} ${point.y}`;
      }
    });
              ...
              (v, i) =>
                fillArea && (
                  <StyledPathArea
                    d={
                      lineStyle == "solid"
                        ? createSolidAreaPath(v)
                        : createSmoothAreaPath(v)
                    }
                    isVisible={isVisible}
          ...
          }
          {
            ...
            lineData.map((v, i) => (
              <StyledPath
                ...
                d={
                  lineStyle == "solid"
                    ? createSolidLinePath(v)
                    : createSmoothLinePath(v)
                }
                isVisible={isVisible}
