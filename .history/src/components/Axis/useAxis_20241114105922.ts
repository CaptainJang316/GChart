...
export const useAxis = ({ 
    ...
    const findAppropriateMaxY = (maxDataValue: number) => {
        ...
        let dataTerm;
        if(maxY >= maxDataValue) {
            dataTerm = maxY - minY;
        } else dataTerm = maxDataValue - minY;
    ...
    const scales = useMemo(() => {
        ...
        const finalmaxY  = findAppropriateMaxY(maxValue);
        const minY = findAppropriateMinY(minY);
    ...
    const yAxis = useMemo(() => {
        tickCount = calculateOptimalTickCount(scales.minY, scales.maxY);
        console.log("tickCnt: " + tickCount);
        let ticks = scales.yScale.ticks(tickCount);

        // 첫 번째 tick이 minY와 다르다면 tick 개수를 조정
        while (Math.abs(ticks[0] - scales.minY) > 0.1) {
            tickCount++;
            ticks = scales.yScale.ticks(tickCount);
        }
    ...
    return { xAxis, yAxis, scales, tickCount };
};
