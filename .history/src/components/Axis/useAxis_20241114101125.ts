...
export const useAxis = ({ 
    ...
    const yAxis = useMemo(() => {
        const tickCnt = calculateOptimalTickCount(scales.minY, scales.maxY);
        console.log("tickCnt: " + tickCnt);
        const ticks = scales.yScale.ticks(tickCnt);
        let ticks = scales.yScale.ticks(tickCount);
        

        // 첫 번째 tick이 minY와 다르다면 tick 개수를 조정
        while (Math.abs(ticks[0] - minY) > 0.1 && tickCount < 10) {
            tickCount++;
