...
export const useAxis = ({ 
    ...
    const scales = useMemo((shouldRound: boolean = false) => {
        ...
        // Y축 스케일 설정
        let maxValue;
        if(shouldRound) maxValue = getAppropriateMaxValue(Math.max(...data));
        else maxValue = Math.max(...data);
    ...
    const yAxis = useMemo(() => {
        ...
        console.log("ticks[ticks.length - 1]: " + ticks[ticks.length - 1]);
        if(ticks[ticks.length - 1] > topPadding) ticks = scales.yScale.ticks(axisCnt);
